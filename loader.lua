...
if shared.VoidDev then shared.VapeDeveloper = shared.VoidDev end
...
local CE_EXECUTORS = shared.CE_EXECUTORS or {
    'solara', 'cryptic', 'xeno', 'ember', 'ronix'
}
...
local function checkExecutor()
    ...
    if getgenv and not global.shared then global.shared = {}
return true end
    if getgenv and not global.debug then global.debug = {traceback = function(string) return string end}
return true end
    if getgenv and not global.require then return true end
    if getgenv and global.require and type(global.require) ~= "function" then return true end

    local executor = "UNKNOWN"
    local res = select(2, pcall(function()
		return string.lower(tostring(identifyexecutor()))
	end))
    if not res then return true end
    executor = res
    if table.find(CE_EXECUTORS, executor) then return true end
...
if CEMode then
	getgenv().cloneref = function(a) return a end
...
for _, folder in
	{ "vape", "vape/games", "vape/profiles", "vape/assets", "vape/libraries", "vape/guis" }
do
...
local SAVE_BLACKLISTED = setmetatable({
	...
	__call = function(self, key)
		if shared.VoidDev then return false end
