    ...
    AutoBalloon = vape.Categories.Utility:CreateModule({
        ...
        Function = function(callback)
            if callback then
                ...
                if not (character and rootPart and balloonController) then
                    AutoBalloon.Toggle(false)
                    return
                end
                
                local lowestpoint = math.huge
                for _, v in pairs(store.blocks) do
                    if v and v.Position and v.Size then
                        local point = (v.Position.Y - (v.Size.Y / 2)) - 50
                        lowestpoint = math.min(lowestpoint, point)
                    end
                end
                
                while AutoBalloon.Enabled and entitylib.isAlive do
                    if rootPart.Position.Y < lowestpoint then
                        local currentBalloons = lplr.Character:GetAttribute('InflatedBalloons') or 0
                        
                        if currentBalloons < 3 then
                            local balloon = getItem('balloon')
                            if balloon then
                                for i = currentBalloons + 1, 3 do
                                    if not AutoBalloon.Enabled then break end
                    ...
                    local waitTime = rootPart.Position.Y < (lowestpoint + 10) and 0.1 or 0.3
                    task.wait(waitTime)
                end
            end
        end,
        Tooltip = 'Automatically inflates balloons when falling into void',
        Priority = 10
    })
end)
